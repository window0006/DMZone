...
const {
      ...
      class LinkEditorExample extends React.Component {
        ...
        getEntityKey() {
          ...
          const entityOffsetInBlock = selection.getStartOffset();
          return block.getEntityAt(entityOffsetInBlock);
        }
        _promptForLink(e) {
          e.preventDefault();
          const {editorState} = this.state;
          const selection = editorState.getSelection();
          if (!selection.isCollapsed()) { // 如果有选取的文字 创建
            var linkKey = this.getEntityKey();
            
            let url = '';
            if (linkKey) {
              // 如果有key 就拿到这个key对应的entity
